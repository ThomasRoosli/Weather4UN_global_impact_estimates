...
def save_hazard_data(hazard: Hazard, metadata: HazardMetadata,
                     source: HazardSource, base_path: str = '') -> tuple[str, str]:
    """
    ...
    file_location_hazard = base_path + '/' + build_file_name_from_hazard(metadata, source, suffix='hazard.hdf5')
    hazard.write_hdf5(file_location_hazard)
    logger.info('Hazard calculated successfully. Stored %s', file_location_hazard)

    file_location_metadata = base_path + '/' + build_file_name_from_hazard(metadata, source, suffix='hazard_metadata.json')
